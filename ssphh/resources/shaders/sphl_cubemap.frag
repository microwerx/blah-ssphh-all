...
{
#ifdef GL_OES_standard_derivatives
	highp vec4 dP1 = dFdx(VS_Position);
	highp vec4 dP2 = dFdy(VS_Position);
	highp vec3 fragN = normalize(cross(dP1.xyz, dP2.xyz));	
#else
	highp vec3 fragN = vec3(0.0, 0.0, 1.0);//normalize(VS_Normal);
#endif
	highp vec3 N = normalize(VS_Normal);
	highp vec3 V = normalize(VS_ViewVector);
	highp float NdotL = dot(N, SunDirTo);
	highp float NdotH = max(0.0, dot(normalize(SunDirTo + V), N));
	highp float cos_theta = 0.5 * (1.0 + NdotL);
	highp vec4 tint = vec4(NdotL, 0.0, 0.0, 0.0);
	if (NdotL < 0.0) {
	...
	if (NdotL > 0.0)
	{
		...
		outputColor += color_d * max(0.0, NdotL);
