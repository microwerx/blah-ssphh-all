...
namespace Fluxions {
	bool DragDropPath(SimpleSceneGraphPtr ssg, RendererContextPtr rendererContext, const std::string& path) {
		// Determine file type
		FilePathInfo fpi(path);
		auto type = FilePathInfo::ExtToFileType(fpi.extension());
		switch (type) {
		case FileType::SCN:
			ssg->reset();
			ssg->Load(fpi.shortestPath());
			return true;
			break;
		case FileType::OBJ:
			ssg->currentTransform.LoadIdentity();
			ssg->addGeometryGroup(fpi.stem(), fpi.shortestPath());
			return true;
			break;
		case FileType::MTL:
			break;
...
namespace SSPHH {
	void SSPHH_Application::OnUpdate(double timeStamp) {
		...
		if (dragDrop.gotDragDrop()) {
			if (dragDrop.gotPaths()) {
				for (auto& p : dragDrop.paths()) { DragDropPath(ssg, rendererContext, p); }
			}
			if (dragDrop.gotText()) { HFLOGDEBUG("DragDrop: text '%s'", dragDrop.text().c_str()); }
			if (dragDrop.gotUnicode()) { HFLOGDEBUG("DragDrop: unicode text '%s'", dragDrop.unicodeText().c_str()); }
			dragDrop.reset();
		}
		...
		if (Interface->renderCoronaCubeMapSCN) { Interface->renderCoronaCubeMapSCN = false; }

		if (Interface->tools.showScenegraphEditor) { SSG_ProcessInterfaceTasks(); }

		if (Interface->tools.showTestsWindow) { Test(); }
		...
		if (!Vf::DearImGuiWidget::ContextInitialized) { Interface->showImGui = false; }
	} // namespace Fluxions
	...
	void SSPHH_Application::DoInterfaceUpdate(double deltaTime) {
		...
		if (!Interface->showImGui) // gamepads[0].IsConnected())
		{
			...
			if (gamepads[0].lthumbAmount() > 0.0f) { gpMoveRate = gamepads[0].lthumbAmount(); }
			else {
				gpMoveRate = 1.0;
			}
			if (gamepads[0].rthumbAmount() > 0.0f) { gpTurnRate = gamepads[0].rthumbAmount(); }
			else {
				gpTurnRate = 1.0;
			}
		}
		...
		if (moveX == 0) Interface->moveX = 0;
		if (moveY == 0) Interface->moveY = 0;
		if (moveZ == 0) Interface->moveZ = 0;
		if (turnX == 0) Interface->turnX = 0;
		if (turnY == 0) Interface->turnY = 0;
		if (turnZ == 0) Interface->turnZ = 0;
		...
		if (Interface->increaseLatitude) { pbskyLatitude += 5.0f * (float)deltaTime; }
		if (Interface->increaseLongitude) { pbskyLongitude += 5.0f * (float)deltaTime; }
		if (Interface->decreaseLatitude) { pbskyLatitude -= 5.0f * (float)deltaTime; }
		if (Interface->decreaseLongitude) { pbskyLongitude -= 5.0f * (float)deltaTime; }
