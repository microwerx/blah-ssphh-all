	...
	void SSPHH_Application::OnUpdate(double timeStamp) {
		...
		if (dragDrop.gotDragDrop()) {
			if (dragDrop.gotPaths()) {
				for (auto& p : dragDrop.paths()) {
					DragDropPath(ssg, rendererContext, p);
				}
			}
			if (dragDrop.gotText()) {
				HFLOGDEBUG("DragDrop: text '%s'", dragDrop.text().c_str());
			}
			if (dragDrop.gotUnicode()) {
				HFLOGDEBUG("DragDrop: unicode text '%s'", dragDrop.unicodeText().c_str());
			}
			dragDrop.reset();
	}
